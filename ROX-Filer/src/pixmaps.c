...
static void masked_pixmap_class_init(MaskedPixmapClass *klass)
{
	GObjectClass *object = G_OBJECT_CLASS(klass);
	object->finalize = masked_pixmap_finialize;
}

static void masked_pixmap_init(MaskedPixmap *mp)
{
	mp->src_pixbuf = NULL;
	mp->huge_pixbuf = NULL;
	mp->huge_width = -1;
...
GType masked_pixmap_get_type(void)
{
	// Check GObject class size properly
	GTypeQuery query;
	...
	if (type == 0)
	{
		static const GTypeInfo info =
		{
			...
			NULL,                              /* base_init */
			NULL,                              /* base_finalize */
			(GClassInitFunc)masked_pixmap_class_init,
			NULL,                              /* class_finalize */
			NULL,                              /* class_data */
			sizeof(MaskedPixmap),
			0,                                 /* n_preallocs */
			(GInstanceInitFunc)masked_pixmap_init,
			NULL                               /* value_table */
		};
