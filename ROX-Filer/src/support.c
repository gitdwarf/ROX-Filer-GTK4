...
const guchar *make_path(const char *dir, const char *leaf)
{
	...
	g_return_val_if_fail(leaf != NULL, "");

	// Free old buffer
	if (buffer)
		g_free(buffer);

	// Use GLib's path building function
	buffer = g_build_filename(dir, leaf, NULL);

	return (const guchar *)buffer;
}
...
gchar *to_utf8(const gchar *src)
{
	...
	/* Check for obviously bad pointer */
	if ((gsize)src < 4096)  /* Likely a bad pointer */
	{
		g_warning("to_utf8: suspicious pointer %p", src);
		return g_strdup("(bad pointer)");
	}

	/* Already valid UTF-8? Just return a copy */
	if (g_utf8_validate(src, -1, NULL))
		return g_strdup(src);

	/* Try locale conversion */
	retval = g_locale_to_utf8(src, -1, NULL, NULL, &error);

	if (error)
	{
		g_warning("to_utf8: locale conversion failed for '%s': %s", src, error->message);
		g_error_free(error);

		/* Just replace bad characters with '?' */
		...
		{
			if (p[i] < 128)  /* ASCII is valid in UTF-8 */
				*out++ = p[i];
			else
				*out++ = '?';
		}
