...
GType view_iface_get_type(void)
{
	...
	if (!iface_type)
	{
		...
		g_print("DEBUG: Registering ViewIface, sizeof(ViewIfaceClass) = %lu\n",
		        (unsigned long)sizeof(ViewIfaceClass));

		iface_type = g_type_register_static(G_TYPE_INTERFACE,
						"ViewIface", &iface_info, 0);

		g_print("DEBUG: ViewIface registered as type %lu\n",
		        (unsigned long)iface_type);
