...
{N_("Copy"),		NULL, (GtkItemFactoryCallback) menuitem_response, MENU_COPY, 	NULL},
{N_("Move"),		NULL, (GtkItemFactoryCallback) menuitem_response, MENU_MOVE, 	NULL},
{N_("Link (relative)"),	NULL, (GtkItemFactoryCallback) menuitem_response, MENU_LINK_REL, NULL},
{N_("Link (absolute)"),	NULL, (GtkItemFactoryCallback) menuitem_response, MENU_LINK_ABS,	NULL},
};
		   ...
		   MaskedPixmap		*image)
{
	...
	g_dataset_set_data_full(context, "full_path",
			g_strdup((const char *) full_path), g_free);
	tmp = (guchar *) encode_path_as_uri((const char *) full_path);
	uri = (guchar *) g_strconcat((const char *) tmp, "\r\n", NULL);
	/*printf("%s\n", tmp);*/
	g_free(tmp);
			...
			gpointer		data)
{
	...
	{
		case	TARGET_RAW:
			path = (guchar *) g_dataset_get_data(context, "full_path");
			if (path && load_file((const char *) path, &to_send, &to_send_length))
			{
...
const guchar *dnd_motion_item(GdkDragContext *context, DirItem **item_p)
{
	...
	if (!item)
	{
		...
		return (const guchar *) drop_dest_dir;
	}
	...
	else
	{
		if (provides(context, text_uri_list) ||
			...
			return (const guchar *) drop_dest_prog;
	}
		...
		if (leafname)
		{
			...
			else
			{
				...
				set_xds_prop(context,
					(const char *) make_path((const char *) dest_uri, leafname));
				       ...
				       FilerWindow	*filer_window)
{
	...
	uris = uri_list_to_glist((const char *) selection_data->data);

	for (next = uris; next; next = next->next)
	{
		guchar	*path;

		path = (guchar *) get_local_path((EscapedPath *) next->data);
		if (path)
		{
			pinboard_pin((const gchar *) path, NULL, x, y, NULL);
			x += 64;
			g_free(path);
		}
			       ...
			       gpointer		user_data)
{
	...
	switch (info)
	{
		...
		case TARGET_RAW:
			...
			break;
		case TARGET_URI_LIST:
			got_uri_list(widget, context, (const char *) selection_data->data,
					time);
			break;
		case TARGET_MOZ_URL:
			...
			guint32             	time)
{
	...
	fd = open((const char *) make_path((const char *) dest_path, leafname),
	...
	else if ((!uri_list->next) && !uri_is_local(uri_list->data))
	{
                         ...
                         (strncasecmp(uri_list->data, "ftp:", 4) == 0))
		{
			run_with_argument(dest_path,
					(const char *) o_dnd_uri_handler.value,
					(char *) uri_list->data);
                }
                else
...
static gboolean spring_now(gpointer data)
{
	...
	dest_path = (const guchar *) g_dataset_get_data(spring_context, "drop_dest_path");
	...
	if (spring_window)
	{
		...
		filer_change_to(spring_window, (const char *) dest_path, NULL);
	...
	{
		spring_window = filer_opendir((const char *) dest_path,
