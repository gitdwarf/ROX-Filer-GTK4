...
GtkWidget *view_collection_new(FilerWindow *filer_window)
{
	...
	GType view_type = view_collection_get_type();
	g_print("DEBUG: view_collection_get_type() returned %lu\n", (unsigned long)view_type);
	g_print("DEBUG: Is it instantiatable? %d\n", G_TYPE_IS_INSTANTIATABLE(view_type));
	g_print("DEBUG: Is it object? %d\n", G_TYPE_IS_OBJECT(view_type));

	view_collection = g_object_new(view_type, NULL);

	g_print("DEBUG: g_object_new returned %p\n", (void*)view_collection);

	if (!view_collection) {
		g_error("Failed to create ViewCollection!");
		return NULL;
	}
...
GType view_collection_get_type(void)
{
	...
	if (!type)
	{
		static const GTypeInfo info =
		{
			...
			NULL,			/* base_init */
			NULL,			/* base_finalise */
			(GClassInitFunc) view_collection_class_init,
			NULL,			/* class_finalise */
			NULL,			/* class_data */
			sizeof(ViewCollection),
			0,			/* n_preallocs */
			(GInstanceInitFunc) view_collection_init
		};
		...
		g_print("DEBUG: GtkViewport class_size = %u, ViewCollectionClass = %lu\n",
		        query.class_size, (unsigned long)sizeof(ViewCollectionClass));
